...
public class RSA {
	...
	public static void main(String[] args) throws Exception {
		RSAKeyPair ret = RSA.generateKeyPair();
		
		//System.out.println(ret.get("privateKey"));
		
		String result = "23,4,56,77,23,1";
		System.out.println("-----------------------------------------");
		System.out.println(String.format("结果为：%s", result));
		//System.out.println(RSA.getPrivateKey(ret.get("privateKey")));
		System.out.println();
		System.out.println("-----------------------------------------");
		System.out.println();
		System.out.println(String.format("RSA public KEY = %s", ret.getPublicKey()));
		System.out.println();
		System.out.println("-----------------------------------------");
		System.out.println();
		String enret = RSA.encrypt(result, ret.getPublicKey());
		System.out.println(String.format("RSA加密结果为：%s", enret));
		System.out.println();
		System.out.println("-----------------------------------------");
		System.out.println();
		System.out.println(String.format("MD5加密结果为：%s", MD5.sign(enret)));
		
		System.out.println(RSA.decrypt(enret, ret.getPrivateKey()));
		
//		for(int i=1;i<31;i++) {
//			System.out.println("DROP TABLE `GAMESERVER`.`T_BETS_STAT_201910" + StringUtils.leftPad(String.valueOf(i), 2, "0") + "`;");
//		}
	}
	/** 指定key的大小 */
	...
	public static RSAKeyPair generateKeyPair() throws Exception {
		...
		return new RSAKeyPair(pri, pub, retValue);
	}
	...
	public static PrivateKey getPrivateKey(String key) throws Exception {
		key = key.replaceAll("(-+BEGIN PRIVATE KEY-+\\r?\\n|-+END PRIVATE KEY-+\\r?\\n?)", "");
	...
	public static boolean checkSign(String content, String sign, String publicKey) {
		...
		{
			KeyFactory keyFactory = KeyFactory.getInstance("RSA");
	        byte[] encodedKey = com.ampletec.commons.codec.binary.Base64.decode2(publicKey);
	        PublicKey pubKey = keyFactory.generatePublic(new X509EncodedKeySpec(encodedKey));
			java.security.Signature signature = java.security.Signature.getInstance("SHA256WithRSA");
			signature.initVerify(pubKey);
			signature.update( content.getBytes("utf-8") );
			return signature.verify( com.ampletec.commons.codec.binary.Base64.decode2(sign) );
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static class RSAKeyPair {
		...
		public RSAKeyPair(String privateKey, String publicKey, String modulus) {
